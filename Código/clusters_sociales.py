...
df_social = df_imputed[social_indicators]

# Reducción de dimensionalidad con PCA
pca = PCA()
pca_result = pca.fit_transform(df_social)
loadings_df = pd.DataFrame(
    pca.components_.T,
    columns=[f"PC{i+1}" for i in range(pca.n_components_)],
    index=df_social.columns,
)
...
for k in k_range:
    ...
    cluster_labels = kmeans.fit_predict(df_social)
    silhouette_avg = silhouette_score(df_social, cluster_labels)
