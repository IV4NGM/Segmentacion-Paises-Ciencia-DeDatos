...
df_gov = df_imputed[gov_indicators]

# Reducción de dimensionalidad con PCA
pca = PCA()
pca_result = pca.fit_transform(df_gov)
loadings_df = pd.DataFrame(
    pca.components_.T,
    columns=[f"PC{i+1}" for i in range(pca.n_components_)],
    index=df_gov.columns,
)
...
for k in k_range:
    ...
    cluster_labels = kmeans.fit_predict(df_gov)
    silhouette_avg = silhouette_score(df_gov, cluster_labels)
...
df_imputed_fixedclusters = df_imputed[gov_indicators].copy()
df_scaled_fixedclusters = df_scaled[gov_indicators].copy()

cluster_labels = kmeans.fit_predict(df_imputed[gov_indicators])
...
# Análisis de las características gubernamentales
cluster_summary_scaled = df_imputed_total.groupby("cluster").mean()
...
#         color="cluster_str",
#         hover_name="country",
#         title="Country Clusters - Government Indicators",
#         locationmode="ISO-3",
#         labels={"cluster_str": "Cluster"},
#     )

#     out_html = (
#         f"{OUTPUT_DIR}/government_country_clusters_{clustersnumber}_clusters.html"
#     )
...
#     members = df_map[df_map["cluster"] == c]["country"].tolist()
#     with open(
#         f"{OUTPUT_DIR}/government_cluster_{c}_countries_{clustersnumber}_clusters.txt",
#         "w",
#     ) as f:
#         f.write("\n".join(members))
